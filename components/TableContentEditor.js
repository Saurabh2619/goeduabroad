import styles from './TableContentEditor.module.css'
...
{data && data.map((i,d)=>{
    ...
    {editMode == d ? <InputField type="text" label={"Heading"} value={i.title} onChange={(e)=>{updateValueAtIndex(d,"title",e)}}/>:<div onClick={()=>{setMode(d)}} className={styles.list + " " + styles.head}>{i.title}</div>}
  {editMode == d  ?  <InputField type="text" data={i.link} label={"Link"} onChange={(e)=>{updateValueAtIndex(d,"link",e)}}></InputField>: <div className={styles.list} onClick={()=>{setMode(d)}}><p >{i.link}</p></div>}
    </div>
...
<div className={styles.item} onClick={()=>{setData(res=>([...res,{title:'Demo Heading',link:'Edit Content'}]))}}>
