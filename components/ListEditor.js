...
{data && data.map((i,d)=>{
    ...
    {editMode == d ? <InputField type="text" label={"Heading"} value={i.heading} onChange={(e)=>{updateValueAtIndex(d,"heading",e)}}/>:<div onClick={()=>{setMode(d)}} className={styles.list + " " + styles.head}>{i.heading}</div>}
  {editMode == d  ?  <InputField type="textarea" label={"Description"} data={i.description} onChange={(e)=>{updateValueAtIndex(d,"description",e)}}></InputField>: <div className={styles.list} onClick={()=>{setMode(d)}}><p >{i.description}</p></div>}
    </div>
...
<div className={styles.item} onClick={()=>{setData(res=>([...res,{heading:'Demo Heading',description:'Edit Content'}]))}}>
