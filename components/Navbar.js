  ...
  const succs = [
    {
      ...
      slug: "/success-stories/visa",
      icon: "/approval.svg",
    },
    {
      title: "Results",
      slug: "/success-stories/results",
      icon: "/results.svg",
    },
  ];
  const links = [
    ...
    {
      title: "IELTS Kit",
      link: "https://app.goeduabroad.com/ielts-online-test-kit",
      dropdown: false,
      //   items: testprep,
    },
    {
    ...
    <>
      {toggle ? (
        <div
          ...
          onMouseEnter={() => {
            (setHovering(true), clearTimeout(timeouta), setTimeoutA(null));
          }}
          onMouseLeave={() => {
            (setHovering(false), setToggle());
          }}
          className={styles.desknav}
          style={{ top: positionModal.y, left: positionModal.x }}
        >
          <ul>
            {links &&
              toggle &&
              links[toggle]?.items?.map((i, d) => {
                return (
                  <Link href={`/${i.slug}`}>
                    <li key={i.title} style={{ animationDelay: `${d * 20}ms` }}>
                      {i.icon ? (
                        <img key={i.title} alt={i.title} src={i.icon} />
                      ) : (
                        ""
                      )}
                      {i.title}
      ...
      <div
        className={
          styles.navcont + " " + (active && !activeToggle ? styles.active : "")
        }
      >
        <Link href={"/"} legacyBehavior>
          <a
            aria-label="Edu Abroad Official Logo"
            className={styles.navlogo + " " + (active ? styles.invert : "")}
          >
            <img
              alt="Edu Abroad Site Logo"
              height={"100px"}
              width={"52.2px"}
              src="/enl.svg"
            />
        ...
        <div className={styles.menuhold}>
          <ul>
                ...
                return (
                  <>
                    <li
                      key={i.title}
                      onClick={(e) => {
                        i.dropdown ? handleToggle(d, e) : "";
                      }}
                      onMouseEnter={(e) => {
                        (setPos(e),
                          i.dropdown ? handleToggle(d, e) : handleClear());
                      }}
                      onMouseLeave={() => {
                        handleOut();
                      }}
            ...
            <li className="mr-2" style={{ visibility: "hidden" }}>
        ...
        <div
          className={
            styles.toggle + " " + (activeToggle ? styles.activetoggle : "")
          }
          onClick={() => {
            activeToggle ? handleSwitchON(false) : handleSwitchON(true);
          }}
        >
          <svg
      ...
      <div
        className={
          styles.mobmenu +
          " " +
          (activeToggle ? styles.activemenu : "") +
          " " +
          (toggle && toggle != undefined ? styles.sub : "")
        }
      >
        <ul>
          {links &&
            links.map((i, d) => {
              return (
                <li className={toggle == d ? styles.togactive : ""}>
                  <a
                    href={i.link}
                    onClick={(e) => {
                      i.dropdown ? handleToggle(d, e) : handleSwitchON(false);
                    }}
                  >
                    {i.title}
                  </a>
          ...
          <li>
            <a className={styles.button} href="https://pte.goeduabroad.com">
              Sign In
            </a>
          </li>
          <li>
