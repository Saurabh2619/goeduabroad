...
function DefaultLayout(props) {
  ...
  function handleAnimateClose() {
    setAnimating(true);
    setTimeout(() => {
      (setAnimating(false), setPromo(false));
    }, 1500);
  }
  ...
  return (
    <div className={styles.maincont}>
      {loading ? (
        <div className={styles.loaderoverlay}>
          <div>
            <CircularProgress
              size="sm"
              classNames={{ svg: "w-6 h-6 text-white fill-white" }}
            ></CircularProgress>
            <p>Loading...</p>
          </div>
        </div>
      ...
      <div className={styles.wa}>
        <div
          className={styles.waicon + " " + (active ? styles.activeIcon : "")}
          onClick={() => {
        ...
        {active ? (
          <div className={styles.wawin}>
            <input
              ...
              placeholder="Send Blank or Enter your Message"
            ></input>
            <Link
              href={`https://wa.me/91${phone}?text=${wa && wa.length > 2 ? encodeURIComponent(wa) : encodeURIComponent("Hi! Want to Enquire.")}`}
              target={"_blank"}
            >
              <button>
                <svg
      ...
      >
        ...
        <Image
          ...
          src="https://res.cloudinary.com/dbnrwsxj1/image/upload/c_scale,q_84,w_1470/v1716230943/campus_f35ygm.webp"
